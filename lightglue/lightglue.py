...
class SelfBlock(nn.Module):
    ...
    def forward(
        ...
        context,attn_avg  = self.inner_attn(q, k, v, mask=mask)
...
class LightGlue(nn.Module):
    default_conf = {
        ...
        "depth_confidence": 0.95,  # early stopping, disable with -1 默认是0.95
        "width_confidence": 0.99,  # point pruning, disable with -1
        # "depth_confidence": 0.98,  # early stopping, disable with -1
        # "width_confidence": 0.99,  # point pruning, disable with -1
        # "depth_confidence": -1,  # early stopping, disable with -1
        # "width_confidence": -1,  # point pruning, disable with -1
        "filter_threshold": 0.005,  # match threshold
        "weights": None,
    }
    ...
    def __init__(self, backbone_out_dim=None,filter_threshold=None,lightglue_pretrain=None,**conf) -> None:
        super().__init__()
        self.conf = conf = SimpleNamespace(**{**self.default_conf, **conf})
        # 如果外部指定了 filter_threshold，则覆盖默认值
        if filter_threshold is not None:
            self.conf.filter_threshold = filter_threshold
        if backbone_out_dim is not None:
            setattr(conf, "input_dim", backbone_out_dim)
        ...
        state_dict = None
        if lightglue_pretrain is not None:
            state_dict = torch.load(lightglue_pretrain,weights_only=True)
              # 替换为你本地权重的路径
    ...
    def _forward(self, data: dict) -> dict:
        ...
        pad_mask0, pad_mask1 = data0.get("mask"), data1.get("mask")  # 获取 mask
        b, m, _ = kpts0.shape
        b, n, _ = kpts1.shape
        device = kpts0.device
        ...
        if do_point_pruning:
            ...
            ind0 = torch.arange(0, m, device=device).unsqueeze(0).expand(b, -1)
            ind1 = torch.arange(0, n, device=device).unsqueeze(0).expand(b, -1)
        ...
        if pad_mask0 is not None:
            desc0 = desc0 * pad_mask0.unsqueeze(-1).float()
        if pad_mask1 is not None:
            desc1 = desc1 * pad_mask1.unsqueeze(-1).float()

        scores, _ = self.log_assignment[i](desc0, desc1)
        m0, m1, mscores0, mscores1 = filter_matches(scores, self.conf.filter_threshold)

        # Apply mask to ensure only valid keypoints are returned##################################################
        if pad_mask0 is not None:
            m0 = torch.where(pad_mask0, m0, torch.full_like(m0, -1))
            mscores0 = torch.where(pad_mask0, mscores0, torch.zeros_like(mscores0))
        if pad_mask1 is not None:
            m1 = torch.where(pad_mask1, m1, torch.full_like(m1, -1))
            mscores1 = torch.where(pad_mask1, mscores1, torch.zeros_like(mscores1))
        ...
        for k in range(b):
            ...
            if do_point_pruning:
                m_indices_0 = torch.gather(ind0[k], dim=0, index=m_indices_0)
                m_indices_1 = torch.gather(ind1[k], dim=0, index=m_indices_1)
        ...
        return {
            ...
            "prune0": prune0,
            "prune1": prune1,
            "self_attn0":self_attn0_all ,
            "self_attn1":self_attn1_all ,
            "cross_attn01": attn01_all ,
            "cross_attn10": attn10_all ,
            "desc0":desc0,
            "desc1":desc1
